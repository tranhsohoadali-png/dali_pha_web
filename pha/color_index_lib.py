...
def _place_detail_numbers(img_white, mask, num_str, draws, rs, size_scale=1.0):
    ...
    mn, me, mx = MIN_TEXT_SIZE * rs * ss, MEAN_TEXT_SIZE * rs * ss, MAX_TEXT_SIZE * rs * ss
    for k in range(1, nc):
...
def index_color(path, debug=False, num_colors=0, min_area=0, smooth=0, design_out=None,
    ...
    try:
        ...
        _L = 0.0
    size_scale = min(2.5, 50.0 / _L) if 0 < _L <= 35 else 1.0
...
def draw_result(edge_img, dpi, width, height, img_name: str = None, debug=False):
    ...
    if img_name:
        ...
        # NGÀY IN: KHÔNG nướng vào file nữa — được đóng dấu lúc GHÉP IN (pha/imposition.py)
        # để tranh trong KHO CŨ (làm trước tính năng ngày) cũng có ngày, và là NGÀY IN THẬT.
