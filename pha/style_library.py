...
# Các nhãn phân loại chuẩn (hiện sẵn để chọn khi tải lên + lọc).
PRESET_CATEGORIES = [
    'Phong cảnh', 'Hoa', 'Chân dung', 'Động vật',
    'Thiết kế', 'Anime / Hoạt hình', 'Tĩnh vật', 'Khác',
]


def categories():
    """Danh sách nhãn ĐANG CÓ ảnh (không rỗng), kèm số lượng."""
...
def category_options():
    """Tất cả nhãn chuẩn (preset) + nhãn lẻ đang dùng, kèm số lượng (0 nếu chưa có)."""
    from django.db.models import Count
    counts = {r['category']: r['n'] for r in
              StyleSample.objects.exclude(category='')
              .values('category').annotate(n=Count('id'))}
    out = [(c, counts.get(c, 0)) for c in PRESET_CATEGORIES]
